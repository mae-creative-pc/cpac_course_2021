...
class Walker {
  ...
  void computeEffect(){
     ...
     float distFromCenter=PVector.sub(this.position, 
                                      CENTER_SCREEN).mag();
     float prevDistFromCenter=PVector.sub(this.prevPosition, 
                                      CENTER_SCREEN).mag();
     float vibrato=map(distFromCenter-prevDistFromCenter, -50, 50, -1, 1);               
     this.vibrato=vibrato;//*0.9+this.vibrato*0.1;
  }
